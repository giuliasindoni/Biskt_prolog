...
%% False node first negation
      ...
      refute(Formulae, [f_nneg, [f_nneg_B1 | Rules1], [f_nneg_B2 | Rules2]]) :-
     ...
     (\+(member(h(S, X), Rest)) ;
      \+( member(X:(Phi =t), Rest))),
        ...
        applying( f_nneg ),
      T = @(nneg(Phi),S),
      refute([h(S, Y), Y:(Phi = t) | Rest], Rules1),
      refute([h(S, T), h(T,T), T: (Phi = t) | Rest], Rules2).

%% True edge first negation
refute(Formulae, [t_eneg, [t_eneg_B1 | Rules1], [t_eneg_B2 | Rules2]]) :-
        ...
        applying( t_eneg ),
        T = @(eneg(Phi),S),
        refute([h(Y, S), Y:(Phi = f) | Rest], Rules1),
        refute([h(T, S), h(T,T), T: (Phi = f) | Rest], Rules2).


%% False implication
refute(Formulae, [f_imp, [f_imp_B1 | Rules1], [f_imp_B2 | Rules2]]) :-
        ...
        applying( f_imp ),
        W1 = @(imp(Phi,Psi),W),
        refute([h(W, Y), Y:(Phi = t), Y:(Psi = f) | Rest], Rules1),
